...
export class WorkoutLogController {
    ...
    static getById = async (req: Request, res: Response) => {
        try {
            ...
            const relatedSession = await sessionRepo.createQueryBuilder("session")
                .where("session.athleteId = :athleteId", { athleteId: log.athleteId })
                .andWhere("CAST(session.date AS DATE) = CAST(:scheduledDate AS DATE)", { scheduledDate: log.scheduledDate })
            ...
            res.json({
                ...log,
                exerciseLogs,
                sessionWorkoutData: relatedSession?.workoutData || null
            });
        } catch (error) {
    ...
    static create = async (req: Request, res: Response) => {
        try {
            ...
            const existing = await repo.findOne({
                where: { athleteId, scheduledDate: normalizedDate }
            });
            ...
            log.scheduledDate = normalizedDate;
            log.status = "in_progress";

            if (programDayId) {
                const day = await programDayRepo.findOne({ where: { id: Number(programDayId) } });
                if (day) {
