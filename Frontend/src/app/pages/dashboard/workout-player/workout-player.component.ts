...
export class WorkoutPlayerComponent implements OnInit, OnDestroy {
    ...
    loadWorkout(id: number): void {
        this.workoutLogService.getById(id).subscribe({
            next: (log: any) => {
                this.workoutLog = log;
                const programDayExercises = Array.isArray(log.programDay?.exercises) ? log.programDay.exercises : [];
                const sessionExercises = Array.isArray(log.sessionWorkoutData?.exercises) ? log.sessionWorkoutData.exercises : [];
                const rawExercises = programDayExercises.length > 0 ? programDayExercises : sessionExercises;

                this.exercises = rawExercises
                    .map((ex: any, index: number) => ({
                        id: ex.id || ex.exercise_id || index + 1,
                        exercise_name: ex.exercise_name || ex.name || 'Exercise',
                        exercise_id: ex.exercise_id || ex.id,
                        exercise_gif: ex.exercise_gif || ex.gifUrl || null,
                        videoId: ex.videoId || null,
                        sets: ex.sets || 3,
                        reps: ex.reps || 12,
                        rest_seconds: ex.rest_seconds || 60,
                        targetWeights: ex.targetWeights || ex.target_weights || [],
                        imageUrl: ex.exercise_gif || ex.gifUrl || (ex.videoId ? `https://img.youtube.com/vi/${ex.videoId}/mqdefault.jpg` : null)
                    }))
    ...
    onExerciseImageError(exercise: any): void {
        if (!exercise) return;
        const ytThumb = exercise.videoId ? `https://img.youtube.com/vi/${exercise.videoId}/mqdefault.jpg` : null;
        if (ytThumb && exercise.imageUrl !== ytThumb) {
            exercise.imageUrl = ytThumb;
            return;
        }
